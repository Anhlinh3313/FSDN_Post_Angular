...
@Component({
  selector: 'app-transfer-reship',
  templateUrl: 'transfer-reship.component.html',
  styles: []
})
export class TransferReshipComponent extends BaseComponent implements OnInit {
  ...
  columns: string[] = ["shipmentNumber", "numPick", "transferAppointmentTime", "orderDate", "fromHubRouting.name",
    "cusNote", "senderName", "senderPhone", "pickingAddress", "transferNote", "shipmentStatus.name"];
  datasource: Shipment[];
  totalRecords: number;
  ...
  loadShipment() {
    ...
    this.datasource = [];
    this.listData = [];
    this.shipmentService.postByType(shipmentFilterViewModel).subscribe(
      x => {
    ...
    setTimeout(() => {
      if (this.datasource) {
        ...
        if (this.selectedStatus) {
          filterRows = filterRows.filter(x => x.shipmentStatus.name === this.selectedStatus);
        }
        if (this.selectedHubRouting) {
          filterRows = filterRows.filter(x => x.fromHubRouting.name === this.selectedHubRouting);
        }
        if (this.selectedDateAppointment) {
          filterRows = filterRows.filter(x => x.transferAppointmentTime === this.selectedDateAppointment);
        }
        //End Custom filter
  ...
  loadFilterLeft() {
    ...
    this.datasource.forEach(x => {
      ...
      if (uniqueStatus.indexOf(x.shipmentStatusId) === -1 && x.shipmentStatusId) {
        ...
        this.statuses.push({ label: x.shipmentStatus.name, value: x.shipmentStatus.name });
      }
      //
      if (uniqueHubRouting.indexOf(x.fromHubRoutingId) === -1 && x.fromHubRoutingId) {
        uniqueHubRouting.push(x.fromHubRoutingId);
        this.hubRoutings.push({ label: x.fromHubRouting.name, value: x.fromHubRouting.name });
      }
    });
  }
  ...
  getSelectedObjectDataRight(): Shipment[] {
    ...
    if (objectDataRight)
      objectDataRight.filter(x => { if (x.key === this.selectedRider.id) arr = x.values; });
    return arr;
  }

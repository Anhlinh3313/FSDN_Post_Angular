...
import {
  ...
  PaymentTypeService, StructureService, UserService, PriceService, ProvinceService, DistrictService, WardService, DeadlinePickupDeliveryService, GeocodingApiService, ServiceDVGTService, RequestShipmentService
} from '../../../services';
...
@Component({
  selector: 'app-create-shipment-excel',
  templateUrl: 'create-shipment-excel.component.html',
  styles: []
})
export class CreateShipmentExcelComponent extends BaseComponent implements OnInit {
  ...
  currentPage: string = Constant.pages.shipment.children.createShipmentExcel.name;
  //
  ngOnInit() {
  ...
  columnsName: string[] = [
    ...
    this.columnNameExcel.province_to,
    this.columnNameExcel.district_to,
    this.columnNameExcel.ward_to,
    this.columnNameExcel.address_note_to,
  ...
  async setDataNew() {
    ...
    if (this.columns.find(f => f.Index == -1)) {
      const notfoutColumns = this.columns.find(f => f.Index == -1);
      this.messageService.add({
      ...
      if (i > 1) {
        ...
        await Promise.all(rows.map(async (cell, j) => {
          ...
          if (column) {
            ...
            } else if (column.Name == this.columnNameExcel.province_to) {
              if (cell) {
                info.provinceName = cell;
              }
            } else if (column.Name == this.columnNameExcel.district_to) {
              if (cell) {
                info.districtName = cell;
              }
            } else if (column.Name == this.columnNameExcel.ward_to) {
              if (cell) {
                info.wardName = cell;
              }
        ...
        this.listShippingAddressImport.push(info);
      }
    })
    //#endregion
    ...
    const users = await this.userService.getByListCodeAsync(cloneUsers);
    this.value = 15;
    //
    ...
    const customers = await this.customerService.getByListCodeAsync(cloneCustomers);
    this.value = 20;
    //
    ...
    // Ward
    customers.map(x => this.listWardID.push(x.wardId));
    let cloneWards = JSON.parse(JSON.stringify(this.listWardID));
    cloneWards = cloneWards.sort().filter((x, i, arr) => arr[i] != arr[i + 1]);
    const wards = await this.hubService.getByListWardIdAsync(cloneWards);
    this.listWardFromExcel = wards;
    this.value = 30;
    //

    // this.listShippingAddressImport = this.listShippingAddressImport.sort().filter((x, i, arr) => {
    //   return arr[i].address != (arr[i + 1] ? arr[i + 1].address : "");
    // });

    await Promise.all(this.listShippingAddressImport.map(async (info, i) => {
      if (!info.provinceName || !info.districtName) {
        let placeRes = await this.geocodingApiService.findFromAddressAsync(info.address);
        if (placeRes) {
          let resInfo = await this.loadLocationToPlaceInfo(placeRes);
          if ((!resInfo.provinceName || !resInfo.wardName) && resInfo.lat && resInfo.lng) {
            let placeRessults = await this.geocodingApiService.findFromLatLngAsync(resInfo.lat, resInfo.lng);
            if (placeRessults && placeRessults.length > 0) {
              let placeResult;
              placeRessults.map(
                m => {
                  if (!placeResult) {
                    if (m.types.indexOf(GMapHelper.ADMINISTRATIVE_AREA_LEVEL_3) !== -1 || m.types.indexOf(GMapHelper.SUBLOCALITY_LEVEL_1) !== -1) {
                      placeResult = m;
                    }
                  }
                });
              if (!placeResult) {
                placeResult = placeRessults[0];
              }
              let resInfoBYLatLng = await this.loadLocationToPlaceInfo(placeResult);
              info.provinceName = resInfoBYLatLng.provinceName;
              info.districtName = resInfoBYLatLng.districtName;
              info.wardName = resInfoBYLatLng.wardName;
              info.lat = resInfoBYLatLng.lat;
              info.lng = resInfoBYLatLng.lng;
            }
          } else {
            info.provinceName = resInfo.provinceName;
            info.districtName = resInfo.districtName;
            info.wardName = resInfo.wardName;
            info.lat = resInfo.lat;
            info.lng = resInfo.lng;
          }
        }
      }
    }));
    ...
    await Promise.all(this.listShippingAddressImport.map(async (info, i) => {
      let infoHubRouting = await this.hubService.getInfoLocationAsync(info.provinceName, info.districtName, info.wardName, null, null, null, null);
      if (infoHubRouting) {
        info.provinceId = infoHubRouting.provinceId;
        info.districtId = infoHubRouting.districtId;
        info.wardId = infoHubRouting.wardId;
        info.hubId = infoHubRouting.hubId;
        info.wardName = infoHubRouting.wardName;
        info.districtName = infoHubRouting.districtName;
        info.provinceName = infoHubRouting.provinceName;
      }
    }));
    //#endregion
    ...
    this.datas.map((rows: string[], i) => {
      if (i > 1) {
        ...
        rows.map((cell, j) => {
          ...
          if (column) {
            ...
            if (column.Name == this.columnNameExcel.staff_code) {
              ...
              const user = this.users.find(f => f.userName.toLowerCase() === this.toLowerCase(shipment.staffCode));
              if (!user) {
                const findUser = users.find(x => x.userName.toLowerCase() == this.toLowerCase(shipment.staffCode));
                if (findUser) {
            ...
            // request code
              ...
              const requestShipment = requestShipments.find(rq => rq.shipmentNumber.toLowerCase() == this.toLowerCase(shipment.requestCode));
              if (requestShipment) {
            ...
            // Phone to
              ...
              if (cell) {
                ...
                shipment.receiverPhone = "0";
              }
            }
            // Address note to
            else if (column.Name == this.columnNameExcel.address_note_to) {
              shipment.addressNoteTo = cell;
            }
            // COD
            ...
            // Customer code
              ...
              const customer = customers.find(f => f.code.toLowerCase() === this.toLowerCase(shipment.customerCode));
              if (customer) {
                setTimeout(() => {
                  this.setDataCustomer(shipment.key, customer);
                }, 0);
              }
            }
          }
        ...
        //load info
        const getInfo = this.listShippingAddressImport.find(f => f.key == shipment.key);
        if (getInfo) {
          shipment.shippingAddress = getInfo.address;
          shipment.toProvinceName = getInfo.provinceName;
          shipment.toProvinceId = getInfo.provinceId;
          shipment.toDistrictName = getInfo.districtName;
          shipment.toDistrictId = getInfo.districtId;
          shipment.toWardName = getInfo.wardName;
          shipment.toWardId = getInfo.wardId;
          shipment.toHubId = getInfo.hubId;
          shipment.latTo = getInfo.lat;
          shipment.lngTo = getInfo.lng;
        }

        // Payment type code
        const paymentType = this.paymentTypes.find(f => f.code.toLowerCase() === (shipment.paymentTypeCode ? shipment.paymentTypeCode.toLowerCase() : ""));
        if (paymentType) {
  ...
  async setDataCustomer(key: Number, cus: Customer) {
    ...
    if (shipment) {
      ...
      await this.initMap(shipment.key, shipment);
    }
  }
  ...
  async uploadExcelNew() {
    ...
    this.shipments.map(x => {
      if (x.shipmentNumber) listShipCode.push(x.shipmentNumber);
    })
    let listShipments = [];
  ...
  async calculator(key: Number) {
    ...
    if (this.data && this.data.endPickTime) model.orderDate = this.data.endPickTime;
    if (model.orderDate && this.data.orderDate) model.orderDate = this.data.orderDate;
    model.cod = ship.cod;
    model.weight = ship.weight;
    model.totalBox = ship.totalBox;
  ...
  SimpleExcel(): void {
    let datas: any[] = [];
    datas.push([
      "#", this.columnNameExcel.request_code, this.columnNameExcel.staff_code, this.columnNameExcel.lading_code, this.columnNameExcel.order_date, this.columnNameExcel.customer_code, this.columnNameExcel.name_to, this.columnNameExcel.phone_to, this.columnNameExcel.address_to, this.columnNameExcel.province_to, this.columnNameExcel.district_to, this.columnNameExcel.ward_to, this.columnNameExcel.address_note_to, this.columnNameExcel.cod, this.columnNameExcel.insured, this.columnNameExcel.weight, this.columnNameExcel.exc_weight, this.columnNameExcel.structure, this.columnNameExcel.service, this.columnNameExcel.type_pay, this.columnNameExcel.content, this.columnNameExcel.cusNote, this.columnNameExcel.type_rider, "Error"
    ]);
    datas.push([
      "stt", "mã bill tổng", "mã nhân viên nhận", "Mã vận đơn", "Ngày vận đơn", "mã khách hàng", "Tên người nhận", "Điện thoại nhận", "Địa chỉ người nhận(google map)", "Tỉnh đến", "Quận/huyện đến", "Quận/huyện đến", "Địa chỉ cụ thể", "Thu hộ(vnđ)", "Khai giá", "Trọng lượng" + environment.unit, "Trọng lượng QĐ", "Mã cơ cấu", "Dịch vụ", "Thanh toán", "Nội dung", "Ghi chú", "Loại xe", "------"
    ]);
    ...
    this.shipments.forEach(item => {
      ...
      datas.push([
        i + 1, this.shipments[i].requestCode, this.shipments[i].staffCode, this.shipments[i].orderDate, this.shipments[i].shipmentNumber, this.shipments[i].customerCode, this.shipments[i].receiverName, this.shipments[i].receiverPhone, this.shipments[i].shippingAddress, this.shipments[i].toProvinceName, this.shipments[i].toDistrictName, this.shipments[i].toWardName, this.shipments[i].addressNoteTo, this.shipments[i].cod, this.shipments[i].insured, this.shipments[i].weight, this.shipments[i].calWeight, this.shipments[i].structureCode, this.shipments[i].serviceCode, this.shipments[i].paymentTypeCode, this.shipments[i].content, this.shipments[i].note, this.shipments[i].typeRider, message
      ]);
      i++;
    });

    // datas = datas.concat(this.shipments);

...
import { Router } from "@angular/router";
  ...
  @Component({
    selector: "detail-shipment-version",
    templateUrl: "detail-shipment-version.component.html",
    styles: [
      `
    agm-map {
      height: 200px;
  ...
  export class DetailShipmentVersionComponent extends BaseComponent {
    ...
    arrObjBoxes: Boxes[] = [];
    shipmentData: ShipmentVersion = new ShipmentVersion();
    ...
    initData() {
      // this.shipmentData = new ShipmentVersion();
    ...
    loadData(shipment: ShipmentVersion) {
      ...
      if (this.shipmentData) {
        ...
        this.serviceDVGTSByshipmentId.getByShipmentId(shipment.shipmentId).subscribe(x => {
          if (!super.isValidResponse(x)) return;
          this.arrObjServiceDVGTs = x.data as ServiceDVGT[];
          if(this.shipmentData.serviceDVGTIds) {
            this.selectedServiceDVGTs = this.shipmentData.serviceDVGTIds;
          } else {
          // this.shipmentData.shipmentServiceDVGTs = this.arrObjServiceDVGTs;
        ...
        this.boxService.getByShipmentId(shipment.shipmentId).subscribe(x => {
          if (!super.isValidResponse(x)) return;
              ...
              x => {
                if (!this.cloneDistrictName) {
                  this.messageService.add({
                    severity: Constant.messageStatus.warn, detail: "Vui lòng chọn Quận/huyện nhận!"
                  });
                  this.messageService.add({
                    severity: Constant.messageStatus.warn, detail: "Vui lòng chọn Phường/xã nhận!"
                  });
                } else {
                  if (!super.isValidResponse(x)) return;
                  let district = x.data as District;
    ...
    loadHubToByWard(selectedWard: number = null) {
      ...
      this.hubService
        .getHubByWardId(selectedWard, includes)
        .subscribe(x => {
          if (!super.isValidResponse(x)) return;
          let stationHub = x.data as Hub;
          if (stationHub == null) return;
    ...
    save(data: ShipmentVersion) {
      // if (!this.isValidToUpdateShipmentVersion())  return;
      ...
      this.shipmentData.totalBox = (this.shipmentData.totalBox) ? this.shipmentData.totalBox : 0;
      ...
      this.shipmentService.update(this.shipmentData).subscribe(
        x => {
          ...
          //create Version
          this.shipmentVersionService.create(data).subscribe(x => {
            if (!super.isValidResponse(x)) return;
      ...
      isValidToUpdateShipmentVersion(): boolean {
    ...
    createBox() {
      ...
      if(this.selectedLength && this.selectedWidth && this.selectedHeight && this.selectedWeight && this.selectedPackType) {
        ...
        obj.shipmentId = this.shipmentData.shipmentId;
    ...
    compareFn(c1: ShipmentVersion, c2: ShipmentVersion): boolean {

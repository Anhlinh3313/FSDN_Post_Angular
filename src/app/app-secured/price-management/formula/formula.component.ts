...
export class FormulaComponent extends BaseComponent implements OnInit {
  ...
  cloneCode: string;
  txtFilterGb: any;
  constructor(private modalService: BsModalService, private formulaService: FormulaService,
  ...
  currentPage: string = Constant.pages.price.children.formula.name;
  modalTitle: string;
  ...
  initData() {
    this.formulaService.getAll().subscribe(
      x => {
        this.datasource = x.data as Formula[];
  ...
  openModel(template: TemplateRef<any>, data: Formula = null) {
    if (data) {
  ...
  save() {
    ...
    if (this.isNew) {
      ...
      this.formulaService.create(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;

        var obj = (x.data as Formula);
      ...
      this.formulaService.update(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;
  ...
  //
  openDeleteModel(template: TemplateRef<any>, data: Formula) {
    this.selectedData = data;
    this.data = this.clone(data);
    this.bsModalRef = this.modalService.show(template, { class: 'inmodal animated bounceInRight modal-s' });
  }
  delete() {
    this.formulaService.delete(new BaseModel(this.data.id)).subscribe(x => {
      if (!this.isValidResponse(x)) return;

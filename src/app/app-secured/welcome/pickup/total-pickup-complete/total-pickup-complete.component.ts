...
@Component({
  selector: "total-pickup-complete",
  templateUrl: "total-pickup-complete.component.html",
  styles: [
    `
          agm-map {
            height: 200px;
...
export class TotalPickupCompleteComponent extends BaseComponent implements OnInit {
  txtFilterGb: any;
  ...
  updateLostPackge(data: Shipment) {
    ...
    if (this.lostPackageNote && this.lostPackageNote.toString().trim() !== "") {
      ...
      this.messageService.add({
        ...
        detail: "Vui lòng nhập lý do mất hàng!"
      });
    }

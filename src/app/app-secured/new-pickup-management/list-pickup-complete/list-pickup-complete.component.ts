...
@Component({
  selector: "list-pickup-complete",
  templateUrl: "list-pickup-complete.component.html",
  styles: [
    `
      agm-map {
        height: 200px;
...
export class ListPickupCompleteComponent extends BaseComponent
  implements OnInit {
  hub = environment;
  ...
  currentPage: string = Constant.pages.pickupManagement.children.listPickupComplete.name;
  modalTitle: string;
  ...
  loadShipment() {
    ...
    this.requestShipmentService
      .getByType(ShipmentTypeHelper.pickupComplete, includes)
      .subscribe(x => {
  ...
  openModelLost(template: TemplateRef<any>) {
  ...
  updateLostPackge() {
    ...
    if (this.lostPackageNote && this.lostPackageNote.toString().trim() !== "") {
      ...
      this.selectedData.forEach(x => {
        x.shipmentStatusId = StatusHelper.pickupLostPackage;
        if (x.note) {
          note = `${this.lostPackageNote}, ${x.note}`;
        } else {

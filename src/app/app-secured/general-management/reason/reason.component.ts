...
import { ReasonService, RoleService } from '../../../services/index';
...
export class ReasonComponent extends BaseComponent implements OnInit {
  first: number = 0;
  txtFilterGb: any;
  constructor(private modalService: BsModalService, private reasonService: ReasonService,
  ...
  currentPage: string = Constant.pages.general.children.reason.name;
  modalTitle: string;
  ...
  columns: string[] = ["code", "name", "pickFail", "pickCancel", "deliverFail", "deliverCancel", "returnFail", "returnCancel"];
  reasonChexbox: string[] = ["pickFail", "pickCancel", "deliverFail", "deliverCancel", "returnFail", "returnCancel"];
  reasonValues: boolean = false;
  datasource: Reason[];
  totalRecords: number;
  ...
  initData() {
    this.reasonService.getAll().subscribe(
      x => {
        this.datasource = x.data as Reason[];
  ...
  openModel(template: TemplateRef<any>, data: Reason = null) {
    if (data) {
  ...
  loadLazy(event: LazyLoadEvent) {
    ...
    setTimeout(() => {
      if (this.datasource) {
        var filterRows;
        var checkReasons;
        checkReasons = this.datasource.filter(x => FilterUtil.gbFilterFiled(x, event.globalFilter, this.reasonChexbox));
        ...
        // sort data
        // console.log(Object.keys(filterRows[0]));
  ...
  save() {
    ...
    if (this.isNew) {
      ...
      this.reasonService.create(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;

        var obj = (x.data as Reason);
      ...
      this.reasonService.update(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;
  ...
  delete() {
    this.reasonService.delete(new BaseModel(this.data.id)).subscribe(x => {
      if (!this.isValidResponse(x)) return;

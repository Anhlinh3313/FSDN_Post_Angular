...
import { ServiceDVGTService } from '../../../services/index';
...
@Component({
  selector: 'app-serviceDVGT',
  templateUrl: 'serviceDVGT.component.html',
  styles: []
})
export class ServiceDVGTComponent extends BaseComponent implements OnInit {
  first: number = 0;
  txtFilterGb: any;

  constructor(private modalService: BsModalService, private serviceDVGTService: ServiceDVGTService,
  ...
  currentPage: string = Constant.pages.general.children.serviceDVGT.name;
  modalTitle: string;
  bsModalRef: BsModalRef;
  displayDialog: boolean;
  data: ServiceDVGT;
  selectedData: ServiceDVGT;
  isNew: boolean;
  listData: ServiceDVGT[];
  //
  columns: string[] = ["code", "name"];
  datasource: ServiceDVGT[];
  totalRecords: number;
  ...
  initData() {
    this.serviceDVGTService.getAll().subscribe(
      x => {
        this.datasource = x.data as ServiceDVGT[];
  ...
  openModel(template: TemplateRef<any>, data: ServiceDVGT = null) {
    if (data) {
  ...
  save() {
    ...
    if (this.isNew) {
      ...
      this.serviceDVGTService.create(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;

        var obj = (x.data as ServiceDVGT);
      ...
      this.serviceDVGTService.update(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;
  ...
  delete() {
    this.serviceDVGTService.delete(new BaseModel(this.data.id)).subscribe(x => {
      if (!this.isValidResponse(x)) return;
  ...
  clone(model: ServiceDVGT): ServiceDVGT {
    let data = new ServiceDVGT();

...
import { ReasonComplainService } from '../../../services/reason-complain.service';
import { ReasonComplain } from '../../../models/reasonComplain.model';
...
@Component({
  selector: 'app-reason-complain',
  templateUrl: './reason-complain.component.html',
  styles: []
})
export class ReasonComplainComponent extends BaseComponent implements OnInit {
  first: number = 0;
  txtFilterGb: any;

  constructor(private modalService: BsModalService, private reasonComplainService: ReasonComplainService,
  ...
  currentPage: string = Constant.pages.general.children.reasonComplain.name;
  modalTitle: string;
  bsModalRef: BsModalRef;
  displayDialog: boolean;
  data: ReasonComplain;
  selectedData: ReasonComplain;
  isNew: boolean;
  listData: ReasonComplain[];
  listRoles: SelectModel[] = [];
  //
  columns: string[] = ["code", "name"];
  datasource: ReasonComplain[];
  totalRecords: number;
  ...
  initData() {
    this.reasonComplainService.getAll().subscribe(
      x => {
        this.datasource = x.data as ReasonComplain[];
  ...
  async loadRole() {
    this.listRoles = await this.roleService.getAllSelectModelAsync();
  }
  ...
  openModel(template: TemplateRef<any>, data: ReasonComplain = null) {
    if (data) {
  ...
  save() {
    ...
    if (this.isNew) {
      ...
      this.reasonComplainService.create(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;

        var obj = (x.data as ReasonComplain);
      ...
      this.reasonComplainService.update(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;
  ...
  mapSaveData(obj: ReasonComplain) {
    if (obj) {
      this.data.id = obj.id;
  ...
  delete() {
    this.reasonComplainService.delete(new BaseModel(this.data.id)).subscribe(x => {
      if (!this.isValidResponse(x)) return;
  ...
  saveClient(list: ReasonComplain[]) {
  ...
  clone(model: ReasonComplain): ReasonComplain {
    let data = new ReasonComplain();

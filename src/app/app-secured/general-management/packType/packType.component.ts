...
import { PackTypeService } from '../../../services/index';
...
export class PackTypeComponent extends BaseComponent implements OnInit {
  first: number = 0;
  txtFilterGb: any;

  constructor(private modalService: BsModalService, private packTypeService: PackTypeService,
  ...
  initData() {
    this.packTypeService.getAll().subscribe(
      x => {
        this.datasource = x.data as PackType[];
  ...
  openModel(template: TemplateRef<any>, data: PackType = null) {
    if (data) {
  ...
  save() {
    ...
    if (this.isNew) {
      ...
      this.packTypeService.create(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;

        var obj = (x.data as PackType);
      ...
      this.packTypeService.update(this.data).subscribe(x => {
        if (!this.isValidResponse(x)) return;
  ...
  delete() {
    this.packTypeService.delete(new BaseModel(this.data.id)).subscribe(x => {
      if (!this.isValidResponse(x)) return;
  ...
  clone(model: PackType): PackType {
    let data = new PackType();
